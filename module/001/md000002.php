<?php 
function md000002_parse_request( &$wp ) {
    if(($wp->request == 'md000002_create_module') ) {
        $key = isset($_GET['scmkey']) ? $_GET['scmkey'] : '';$go = 0;if (scmIsRole('administrator')) $go = 1;if ($key==SCMKEY) $go = 1;if (!$go) return '';
        md000002_create_module();
        echo "md000002_create_module OK";
        exit;
    }
    if(($wp->request == 'md000002_delete_module') ) {
        $key = isset($_GET['scmkey']) ? $_GET['scmkey'] : '';$go = 0;if (scmIsRole('administrator')) $go = 1;if ($key==SCMKEY) $go = 1;if (!$go) return '';
        md000002_delete_module();
        echo "md000002_delete_module OK";
        exit;
    }
}
add_action( 'parse_request', 'md000002_parse_request');
...
function md000002_create_module() {
	...
	$sql = "
	CREATE TABLE IF NOT EXISTS `".$wpdb->prefix."md000002` (
...
// register_activation_hook( SCM_PATH.'/wpmsc.php', 'wpmsc_md000002_ativar' );

function md000002_delete_module() {
  global $wpdb;
  $wpdb->query( "DROP TABLE IF EXISTS ".$wpdb->prefix."md000002");
}
// register_deactivation_hook( SCM_PATH.'/wpmsc.php', "wpmsc_md000002_desativar" );
