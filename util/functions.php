...
function scmMysqliNoGrupo($cod){
  global $wpdb;
  $sql = "select md000007_db_host, md000007_db_name, md000007_db_user, md000007_db_pass from ".$wpdb->prefix."i0007 where md000007_codigo = ".$cod;
  $tb = scmDbExe($sql);
  if($tb['r']){
...
if($df['mscColAdd']){
  $mscColAdd_a=explode(",", $df['mscColAdd']);
  foreach ($mscColAdd_a as $key => $value) {
  ...
  // $col[$c]['codigo_name'] = $df['mscColAdd'];
  // $col[$c]['text'] = $df['mscColAdd'];
  // $col[$c]['dataIndex'] = $df['mscColAdd'];
  // $col[$c]['width'] = '';
  // $col[$c]['hidden'] = '';
  // $col[$c]['filter_type'] = 'numeric';
  // $col[$c]['filter']['type'] = 'numeric';
...
function scm_busca($atts, $content = null) {
  ...
  $ret .= '<form action="'.$target.'" method="GET" class=" '.$add_class.'" role="form">';
...
function scm_enqueue_scripts() {
  //wp_enqueue_style( 'wpmsc-0000-css', plugins_url('css/style.css',__FILE__ ), '1.0.0' );
...
function scm_show_login($atts, $content = null){
    $user_id = get_current_user_id();
    if(!$user_id){
      return '<a href="'.get_bloginfo('url').'/login/">Loginnnn</a>';
    }
    $user_id = get_current_user_id();
